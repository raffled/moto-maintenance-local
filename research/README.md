...
| 6 | [retrieval_validation.ipynb](retrieval_validation.ipynb) | Validates `agent/retrieval.py` — demonstrates the disambiguation flow (oil change query returning both engine-assembly Ch.18 and maintenance Ch.22 results), `retrieve_from_section()` scoping the walk to the user's chosen chapter, BFS dependency walk, image path deduplication, torque spec passthrough, and ordering verification across four representative queries. |
| 7 | [planner_validation.ipynb](planner_validation.ipynb) | Validates `agent/planner.py` — inspects the prompt context block, runs a simple single-section plan (handlebar adjustment), a multi-section plan with prerequisites (fork cartridge disassembly), and the full disambiguation flow for the oil change use case. Shows torque spec and image path passthrough from chunk metadata. |
